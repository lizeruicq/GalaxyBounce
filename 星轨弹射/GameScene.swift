...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func createPlayerShip() {
        ...
        playerShip.physicsBody?.usesPreciseCollisionDetection = true // 启用精确碰撞检测
...
//                    self?.showLevelCompleteMenu()
        ...
        if (bodyA.categoryBitMask == playerCategory && bodyB.categoryBitMask == asteroidCategory) ||
           (bodyA.categoryBitMask == asteroidCategory && bodyB.categoryBitMask == playerCategory) {
            ...
            // 应用回弹力，增加反弹力度
            let bounceForce: CGFloat = 100.0 // 增加反弹力度
            playerShip.physicsBody?.applyImpulse(CGVector(
                dx: -normalizedDx * bounceForce,
                dy: -normalizedDy * bounceForce
            ))
            ...
            // 确保飞船不会穿透陨石
            let overlap = contact.collisionImpulse
            if overlap > 0 {
                let correction = CGVector(dx: normalizedDx * overlap, dy: normalizedDy * overlap)
                playerShip.position = CGPoint(
                    x: playerShip.position.x + correction.dx,
                    y: playerShip.position.y + correction.dy
                )
            }
        }
    ...
    private func createAsteroids() {
        ...
        for _ in 0..<asteroidCount {
            // 随机大小（玩家飞船的1-1.5倍）
            let baseSize = CGFloat.random(in: 50...80)
            ...
            // 创建基础不规则形状
            let basePath = createIrregularPath(size: baseSize)
            let baseShape = SKShapeNode(path: basePath)
            baseShape.fillColor = SKColor(red: 0.3, green: 0.3, blue: 0.3, alpha: 1.0)
            baseShape.strokeColor = SKColor(red: 0.4, green: 0.4, blue: 0.4, alpha: 1.0)
            baseShape.lineWidth = 2
            asteroid.addChild(baseShape)
            
            // 添加表面纹理
            let textureCount = Int.random(in: 3...5)
            var existingTextures: [(position: CGPoint, size: CGFloat)] = []
            
            for _ in 0..<textureCount {
                var validPosition = false
                var attempts = 0
                let maxAttempts = 30 // 增加最大尝试次数
                
                while !validPosition && attempts < maxAttempts {
                    let textureSize = CGFloat.random(in: baseSize * 0.05...baseSize * 0.1)
                    
                    // 随机位置
                    let angle = CGFloat.random(in: 0...(CGFloat.pi * 2))
                    let distance = CGFloat.random(in: 0...(baseSize * 0.4))
                    ...
                    // 检查是否与现有纹理重叠
                    var overlaps = false
                    for existing in existingTextures {
                        let dx = position.x - existing.position.x
                        let dy = position.y - existing.position.y
                        let distance = sqrt(dx * dx + dy * dy)
                        // 增加间距要求，使用1.5倍的安全系数
                        if distance < (textureSize + existing.size) * 1.5 {
                            overlaps = true
                            break
                        }
                    }
                    ...
                    if !overlaps {
                        let texture = SKShapeNode(circleOfRadius: textureSize)
                        texture.position = position
                        
                        // 设置纹理颜色
                        let grayValue = CGFloat.random(in: 0.2...0.4)
                        texture.fillColor = SKColor(red: grayValue, green: grayValue, blue: grayValue, alpha: 1.0)
                        texture.strokeColor = SKColor.clear
                        
                        asteroid.addChild(texture)
                        existingTextures.append((position: position, size: textureSize))
                        validPosition = true
                    }
            ...
            // 添加高光效果
            let highlight = SKShapeNode(circleOfRadius: baseSize * 0.1)
            highlight.fillColor = SKColor(red: 0.5, green: 0.5, blue: 0.5, alpha: 0.3)
            highlight.strokeColor = SKColor.clear
            highlight.position = CGPoint(x: -baseSize * 0.2, y: baseSize * 0.2)
            asteroid.addChild(highlight)
            
            // 添加旋转动画
            let rotateAction = SKAction.rotate(byAngle: .pi * 2, duration: Double.random(in: 20...30))
            ...
            // 设置物理体
            asteroid.physicsBody = SKPhysicsBody(polygonFrom: basePath)
            ...
            asteroid.physicsBody?.allowsRotation = false // 防止旋转
            asteroid.physicsBody?.usesPreciseCollisionDetection = true // 启用精确碰撞检测
